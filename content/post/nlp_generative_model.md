...
generator greediliy ouptut the word that maximize the reward. I will first go through methods proposed to alleviate the first issue then the second one. 
...
<figcaption align="middle">Overview of LeakGAN. The discriminator and generator are in the up and bottom dottred box. The generator contains Manager and Worker module. The Manager forward information computed from discriminator to help worker to take action.</figcaption>
</figure>

Given generator $G\_{\theta}$ (bottom dotted-line) and $D\_{\phi}$ (upper dotted-line), the generator is broken into Manager ($\mathcal{M}$) and Worker module ($\mathcal{W}$). To generate next word $x\_{t+1}$, the current sentece $S\_{t}$ is first fed into feature extratctor $\mathcal{F} $inside $D\_{\phi}$. The extracted feature then *leak* to manager in the generator to compute a goal vector projected to action space $w\_{t}$ by a linear projection $\psi$. The worker comptute the probability distribution of next action to take $O\_{t}$ given the previos word $x\_{t}$ then adjust its distribution by element-wise dot with $w\_{t}$ to get final distribution. 
...
<figcaption align="middle">Left: Performance of LeakGAN compaired with other baselines on synthetic dataset. Right: From top to bottom: Performance of LeakGAN comparied with other baselines on Chinese Poem (short), COCO-caption (medium), EMNLP2017 WMT (long) dataset.</figcaption>
</figure>
...
<figcaption align="middle">Comparisoin of feature trace of LeakGAN to other baselines. Text generated by LeakGAN is close to real data when the generataion process is completed.</figcaption>
</figure>
...
_HOW DO WE EVALUATE OUR TEXT GENERATION MODEL?_
...
<figcaption align="middle">MaskGAN overview: The encoder encode the masked text input and generator try to learn to fill in the blank by reward from discriminator. The correct answer of example above is a,b,c,d,e. </figcaption>
</figure>


The goal of generator is to predict the token given the masked sentence and what it has filled in. Different from the prior works, the discriminator here takes the generatd text _and_ masked sentence as input to output a scalar to determine fake or real in every time step. An advantage of this design is the generator only get more punished on the error token which causes the entire sentence from real to fake. The tokens generated before this token will not be punished. The reason of why incldue masked input in discriminator is to help discriminator to deal with the following case as illustrated in the paper: If the generator output _the director director guided the series_, it is ambiguous of which director is fake because _the director_ **expertly** _guided the series_ and _the_ **associate** _director guided the series_ are valid sentence. The masked input can tell the discriminator which contetx of the real sentence. 

Formally, given a input sequence $\boldsymbol{x} = (x\_1, \cdots, x\_T)$ and a binary mask of same length $\boldsymbol{m} = (m\_1, \cdots, m\_T)$. The generator $G\_{\theta}$ and $D\_{\phi}$ are
\begin{align}
...
<figcaption align="middle">Left: Human Preference score of paired comparison on IMDB review datasset. Right: Human Preference socre of paried comparison on PTB dataset.</figcaption>
</figure>
