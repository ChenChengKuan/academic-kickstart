...
The above sample is the best we can get from training GAN to generate text from scratch. But what could we get if we try not to be too harsh to our model and allow it to be pre-trained first on some corpus? 
...
**Takeaways**
Applying standard GAN framework to discrete space is difficult due to the lack of gradient information from the discriminator to guide the training. To generate some meaningful character pattern, the state-of-the art methods rely on either continous relaxation of discrete distribution (Gumbel-softmax) or add gradient penaly to WGAN. Generating meaningful setnences under GAN framework is possible with pretraining and other techniques.
