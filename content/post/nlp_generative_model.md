...
Concurrently, Semeniuta et al. propose a convolutional-deconvolutional VAE with recurrent models on top of the output of deconlolutional layers for text generation. They further introduce an auxilary loss $J_aux = -\mathbb{E}log\_{q(\boldsymbol(z)|\boldsymbol{x})}p_{\phi}(\boldsymbol{x}|\boldsymbol{z})$ into the optimization of ELBO to force the decoding process rely on the latent representation $\boldsymbol{z}$. 
